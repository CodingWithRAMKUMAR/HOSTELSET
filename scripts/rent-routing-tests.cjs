...
test('rent reminder worker repairs before-due and due-today queue rows without resending completed reminders', () => {
  ...
  assert.match(repositorySource, /rent_record_received_amount/)
  assert.match(repositorySource, /isMonthlyRentPaymentMethod/)
...
test('rent reminder dry-run is authorized, read-only, and does not call Brevo', () => {
  ...
  assert.match(dryRunSource, /canonicalReceivedAmount/)
  assert.match(dryRunSource, /cycleFullyPaid/)
  assert.match(dryRunSource, /linkedSuccessfulPaymentIds/)
  assert.match(dryRunSource, /rent_cycle_paid/)
